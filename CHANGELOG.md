...
## [Unreleased]
- **Site auto-reset API round-trip fix (#643)**: `handleGetSite` now reads the normalized fields returned by `getUserSite`/`getUserSiteById` instead of raw column names, so `autoReset.enabled`, `isDraft`, `updatedAt`, `publishedAt`, `passwordProtected` and the archive timestamps/counts are no longer undefined on the wire; both helpers surface `publishedAt`/`passwordProtected`.
- **Dashboard redesign, Wave 3 suite (#644–#649)**: Engagement + Rewards UX cleanup with narrow-table overflow and mobile raffle-row fixes (#644), Audience + Analytics body redesign with a member-history drawer in the audience panel (#645), Site Settings + Account UX cleanup with settings runtime regression fixes (#646), Telegram UX polish (ghost button paint, mobile overflow, touch targets, API-error copy, contained table scrollers) (#647), a fix preserving Engagement styles across SPA navigation (the fragment endpoint now reports destination stylesheets and the dynamic-section loader makes them usable before markup is shown) (#648), and the Wave 3 final responsive/accessibility/consistency pass (#649).
