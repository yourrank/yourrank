...
- **Community viewer and rewards improvements**: optional Spotlight presentation, independent points-only event leaderboards, channel navigation and contextual help, clearer Site settings and Insights, private R2 reward pictures, and persistent reward deletion that retains Claims history. Corrected event-link fallback, search pagination, small-board podiums, and in-canvas editing. R2 buckets and the additive migrations must be prepared before deployment.
- **Site auto-reset API round-trip fix (#643)**: `handleGetSite` reads the normalized site model so publication, draft, password protection, reset and archive fields round-trip correctly.
- **Dashboard redesign, Wave 3 suite (#644–#649)**: Rewards, Audience, Analytics, Site Settings, Account and Telegram usability refinements; destination stylesheets load before SPA fragments appear; responsive and accessibility corrections across the shared workspace. Legacy Engagement styling fixes remain historical changes, not new product scope.
