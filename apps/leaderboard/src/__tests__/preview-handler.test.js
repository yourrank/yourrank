...
describe("handleDashboardPreview", () => {
  ...
  it("edits only the creator name and tagline, never the leaderboard heading", async () => {
    ...
    expect(home).toContain('<h1 class="viewer-hero-name" data-preview-field="f_name">Northstar</h1>');
    ...
    expect(leaderboard).toContain('<h1 class="viewer-h1">Leaderboard</h1>');
    expect(leaderboard).toContain('class="viewer-rail-name" data-preview-field="f_name"');
    expect(leaderboard).not.toContain('viewer-hero-name');
  });
