...
describe("giveaway draw and timeout", () => {
  ...
  it("records history and deadline for eligible draws", async () => {
    const writes = [];
    const run = async (sql, params) => { if (sql.startsWith("SELECT")) return [eligible]; writes.push({ sql, params }); return []; };
    expect(await drawGiveaway(run, { ...session, rules: { winnerMustRespond: true, responseTimeout: 30 } })).toEqual({ winnerId: "e1" });
    expect(writes[0].sql).toContain("chat_giveaway_draws");
    expect(writes[1].params).toEqual([id, "e1", true, 30]);
  });
