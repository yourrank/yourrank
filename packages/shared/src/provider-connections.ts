...
export interface CommunityChannelInput {
  ...
  /** Ownership already proven by the caller (creator id === channel id). */
  verified: boolean;
}
...
export async function loadCreatorConnection(run: SqlRunner, userId: string, provider: ProviderId): Promise<CreatorConnection | null> {
  const [row] = await rows<{
    external_user_id: string; username: string | null; linked_at: string | null; token_expires_at: string | null;
    has_access: boolean; has_refresh: boolean;
  }>(run,
    `SELECT external_user_id, username, linked_at, token_expires_at,
...
/** Upsert the creator's connection; raises 23505 if another user actively owns the external id. */
export async function linkCreatorConnection(run: SqlRunner, input: CreatorConnectionInput): Promise<void> {
  await run(
...
export async function loadCommunityChannel(run: SqlRunner, siteId: string, provider: ProviderId): Promise<CommunityChannel | null> {
  const [row] = await rows<{ external_channel_id: string; external_channel_name: string | null; linked_at: string | null; verified_at: string | null }>(run,
    `SELECT external_channel_id, external_channel_name, linked_at, verified_at
...
/** Bind a channel to a site; raises 23505 if another site actively owns the channel. */
export async function linkCommunityChannel(run: SqlRunner, input: CommunityChannelInput): Promise<void> {
  await run(
    `INSERT INTO community_channels AS ch
     ...
     VALUES ($1, $2, $3, NULLIF($4, ''), now(), CASE WHEN $5 THEN now() END)
...
/**
 * Other sites of `userId` still bound (active + verified) to the channel the
 * creator's active connection owns. Used to decide whether disconnecting one
 * site must also revoke the creator connection.
 */
export async function otherVerifiedChannelForCreator(
  ...
  const [row] = await rows<{ id: string }>(run,
    `SELECT ch.site_id AS id
       ...
       JOIN creator_connections cc ON cc.user_id = s.user_id AND cc.provider = ch.provider
      WHERE s.user_id = $1 AND ch.provider = $2 AND ch.site_id <> $3
      ...
      LIMIT 1`,
    [userId, provider, excludeSiteId]);
  return row?.id ?? null;
}

/**
 * Routing for inbound provider events. A channel routes to its site only when
 * the binding is active AND verified AND the site owner's active creator
 * connection is for the same external id (creator owns the channel). Locks the
 * site row for the caller's transaction.
 */
export async function resolveVerifiedCommunityChannel(
  ...
  const [row] = await rows<{ site_id: string; user_id: string }>(run,
    `SELECT s.id AS site_id, s.user_id
       FROM community_channels ch
       JOIN sites s ON s.id = ch.site_id
       JOIN creator_connections cc ON cc.user_id = s.user_id AND cc.provider = ch.provider
