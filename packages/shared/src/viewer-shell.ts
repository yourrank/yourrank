...
// The bell and its panel are inert without site-shell.js, which fills the
// list, the unread badge and the read actions from /api/viewer/notifications.
export function viewerNotificationBell(): string {
  return `<div class="viewer-notify" id="viewer-notify" hidden>` +
    `<button class="viewer-icon-button viewer-notify-bell" id="viewer-notify-bell" type="button" aria-label="Notifications" title="Notifications" aria-haspopup="dialog" aria-expanded="false" aria-controls="viewer-notify-panel">${viewerIcon('bell')}<span class="viewer-notify-badge" id="viewer-notify-badge" hidden aria-hidden="true"></span></button>` +
    `<div class="viewer-notify-panel" id="viewer-notify-panel" role="dialog" aria-labelledby="viewer-notify-title" hidden>` +
    `<div class="viewer-notify-head"><h2 id="viewer-notify-title">Notifications</h2><button type="button" class="viewer-notify-readall" id="viewer-notify-readall" hidden>Mark all as read</button></div>` +
    `<p class="viewer-notify-status" id="viewer-notify-status" role="status" aria-live="polite"></p>` +
    `<ul class="viewer-notify-list" id="viewer-notify-list" role="list"></ul>` +
    `</div></div>`;
}
...
${signedIn ? viewerNotificationBell() : ''}<a class="viewer-user" id="viewer-top-avatar" href="${esc(guest ? signInHref : `${accountHref}#vd-profile`)}" aria-label="${guest ? 'Sign in to your viewer account' : `Viewer account${viewerName ? `: ${esc(viewerName)}` : ''}`}" title="${guest ? 'Sign in' : 'Open viewer account'}"><span class="viewer-user-avatar" id="viewer-top-mark">${(!guest && viewerMark) || viewerIcon('user')}</span><span><strong id="viewer-top-name">${esc(guest ? 'Sign in' : viewerName || 'Account')}</strong>${balance != null ? `<small data-credit-balance="${Number(balance) || 0}"><span data-credit-balance-num>${Number(balance).toLocaleString("en-US")}</span> credits</small>` : ''}</span>${viewerIcon('down')}</a></div></header>
