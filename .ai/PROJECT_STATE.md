...
Verified: 3 new behavior tests plus 6 existing dialog tests PASSED; browser proved focus on opening, both Tab boundaries, Escape restoring the profile summary, reopened drawer and background accessibility recovery. Full suite 2252 PASSED / 109 SKIPPED / 0 FAILED; lint and final 87-asset build PASSED. Typecheck passed in the preceding batch; this follow-up changes the vanilla JS client/CSS only. Logs: .local/ui-runtime/help-tests.log, help-lint.log, help-build.log. An unchanged retry passed after the known static-scan test exceeded its existing 5-second deadline. The delayed account-delete cleanup remains an unverified follow-up; the overall audit, actual email delivery and real Polar flows remain incomplete. No deployment or commit.

## Viewer feedback receipt and support contract — 2026-09-10

Site settings now contain an owner-only Feedback tab backed by the existing viewer_feedback table. GET/PATCH /api/site/feedback require authentication, explicit site identity and canRoleManageSiteSettings. The inbox supports All/Unread, explicit read/unread, refresh, bounded pagination, loading/error/empty states and text-only message rendering. No schema, route-registry or identity migration. Cursors use a scoped SQL row lookup: real HTTP testing caught timestamp serialization losing microseconds and skipping the next page; the final path keeps the comparison inside Postgres.

Public feedback explicitly goes to the site owner with no personal reply channel. YourRank support retains its existing stored-message/email model; the drawer no longer promises tickets, a two-hour response, or attached diagnostics. Its context now matches the server enum (the old URL value rejected both forms), its minimum length matches the server, unavailable account email blocks sending instead of using a fake address, and requests release controls after a deadline. Stored contact messages remain acknowledged if their email notification fails. No automatic streamer notifications or conversation/reply platform was added.

Executed: full root tests 2249 PASSED / 109 SKIPPED / 0 FAILED; lint, typecheck and final build (87 assets) PASSED. Final targeted CSS/feedback/contact tests: 60 PASSED. Isolated HTTP journey verified anonymous public submission, owner receipt, unauthenticated/cross-site denial, 28 messages over two pages with equal microsecond timestamps, and read/unread persistence. Browser verified desktop/mobile (390px, body 380px), Arabic and literal HTML text, unread filtering/empty state/focus recovery, support form receipt in SQL and navigation to Home without a dirty prompt. Final message styles computed at 15px with intended spacing; no console errors observed. Temporary test accounts and exact browser fixture records were removed. Evidence logs: .local/ui-runtime/feedback-*.log; HTTP runner: .local/ui-runtime/verify-feedback.ts.

Not verified: actual email delivery/admin reply arrival, real Polar configuration/payment, complete remaining audit branches. No commit, push or production deployment. An initial full run exposed the intentional public-copy snapshot/assertion changes and a static-scan timeout under competing work; final unchanged gates passed. A transient Windows asset-file lock cleared on retry. Existing help drawer focus trapping/return focus and delayed account-deletion behavior observed during a local cleanup remain follow-up audit items; this batch does not certify them.

## Streamer journey audit and Polar verification — 2026-09-09

The first audit batch executed save/cancel/discard/validation and selector recovery in the local browser, plus selected real HTTP journeys for player validation, publish/unpublish, export-unavailable state and Moderator Insights/connection permissions. Found and fixed two regressions: edits made during an in-flight save were marked clean, and cancelling site selection retained a misleading selected label. Draft revisions now preserve newer changes and prevent navigation from abandoning them; context selectors keep the committed site and do not dirty content. Polar redirects now use the shell's existing navigation guard. Checklist/evidence: docs/STREAMER_JOURNEY_AUDIT_CHECKLIST.md. This is not completion of all 46 checks; feedback inbox/support conversation work remains open.

Owner selected Polar with products/secrets to connect later. Source implements configuration-gated checkout/portal, authenticated identity, product validation, signed webhooks, replay protection, current-state reconciliation, durable checkout reservations, deletion guards and payment/subscription persistence. Setup: docs/POLAR_BILLING_SETUP.md. The three unshipped migrations starting at 20260909120000 were amended to comply with the unchanged expand gate: nullable unique provider keys, existing private backend schema for Polar tables, explicit yourrank_app grants. Local test DB yourrank_ui_test_20260909 has all fields/tables; prior empty public Polar tables were aligned transactionally. No production changes. Rollback retains additive records. Do not reapply these migrations blindly to the already aligned local DB.

Final evidence: root test suite 2241 PASSED / 109 SKIPPED / 0 FAILED; lint, typecheck, build (86 assets), migration compatibility and diff check PASSED. Separate Polar SQL/boundary run: 14 PASSED / 73 assertions; combined with migration tests: 24 PASSED / 132 assertions. Four selected HTTP journeys PASSED / 51 assertions. Private schema permissions verified: anon/authenticated cannot USAGE; yourrank_app can access both tables. Billing/Usage desktop/mobile and corrected Insights mobile composition verified. Source and generated bundle are synchronized. A concurrent full-test attempt exceeded the existing static-scan deadline; the unchanged suite passed when run without the competing build/lint workload.

Real Polar checkout/webhook delivery/portal/invoices remain NOT VERIFIABLE until configuration. Full export download needs ACCOUNT_EXPORTS; current local route truthfully reports unavailable. Full role/site matrix and all remaining checklist branches are not certified. No commit, push or deployment. Preserve the earlier UI recovery work below.

## Dashboard UI recovery locally verified — 2026-09-09

Branch `codex/dashboard-ux-recovery`, based on deployed `8e436927`. Implements explicit save results, complete discard via saved-document reload, visible toasts/body-portal dialogs, Site preview initialization/readiness and stale-frame guards, route headings/breadcrumbs, community identity on Home, and progressive disclosure for Activities automation. No route/schema/domain migration or production deployment. The historical audit is `docs/UI_UX_DIAGNOSTIC_2026-09-08.md`; current acceptance, executed evidence and remaining work are in `docs/UI_UX_EXECUTION_PLAN.md`.

The usage interruption was resolved and execution resumed. Final root tests: **2,223 PASSED / 108 SKIPPED / 0 FAILED**; root lint and typecheck PASSED, generated assets rebuilt, diff whitespace check PASSED. Actual isolated local Worker/browser checks confirmed invalid-save recovery, valid persistence, complete discard, visible toast, Escape/Back recovery, first Site preview/Refresh, invalid preview document recovery, mobile heading/overflow and shared drawer closure. Home now retains actual player scores; Rewards/Connections secondary detail is disclosed on demand; Activities keeps existing templates visible. Verification limits and exact evidence are in the execution plan. No production deployment, push or commit occurred; external services/roles and production latency remain unverified.
...
- Safe engagement remains part of the launch product. Creator-branded `/<slug>/me` owns authenticated free code-drop claiming, including the successful-action path that creates a Membership and records Participation. Global `/me` remains an index; restricted giveaway, raffle, prediction, wagering, game and tournament operations remain absent from primary product surfaces.
...
- The creator-scoped My Community renderer also owns the free code-drop claim form. A signed-in non-member may submit a valid shared code; only a successful atomic claim creates the Membership and then appears through the existing Participation projection. Blocked Memberships cannot claim. The removed global `/me` creator-detail/drop form and `/api/viewer/site` endpoint stay removed.
...
## Viewer persistent shell and information hierarchy — 2026-09-14

Supported viewer routes now use one progressively enhanced shell across community Home, Leaderboard, Reward shop, My activity, global My communities, Viewer account, and viewer Help. Canonical server-rendered documents remain the route source and no-JavaScript fallback; `viewer-app.js` replaces center content, updates history and page controllers, retains the selected-community context, and falls back to full navigation outside supported viewer documents. Restricted Games remains outside this shell.

Home leads with real community Credits, explicit unlinked leaderboard-player state, claim count, next-reward progress, verified Kick live state when a configured Kick URL is available, and recent credit activity. Completed onboarding collapses automatically. Reward actions, empty states, leaderboard participation guidance, code language, community lookup, and top-right controls now reflect the viewer's actual state. Viewer Account, Membership, and Leaderboard Player identities remain separate; no rank or live state is inferred from a name.

Local synthetic browser verification covers desktop/mobile rendering, persistent shell and selector identity through community/account/help routes and browser Back/Forward, context retention, completed-guide collapse, failed-navigation recovery, reward/code mutations with CSRF and idempotency, and signed-out/empty/unavailable/blocked states. Production, real OAuth, database persistence, and deployed behavior remain unverified.
