...
**Environment identity guard (extends P1-03).** `scripts/release-target-guard.mjs <staging|production>` now runs before every workflow mutation step — production `release-preflight`/`migrate`, staging `staging-preflight`/`migrate-staging`, and `staging-bootstrap.yml`. It verifies `RELEASE_ENVIRONMENT`, the git ref/event, Supabase project refs, Hyperdrive ids, and database URLs against the known production identifiers and fails closed, reporting variable names only. `deploy.yml` additionally refuses any `workflow_dispatch` from a non-`main` ref, and `staging.yml` can now run from a same-repo pull request labeled `deploy-staging` (CI gate first, then the full release topology plus an `e2e-staging` run against the deployed apex). Note the app reaches Postgres over Hyperdrive only — there are no Supabase anon/service-role keys in this system, so no `STAGING_SUPABASE_URL`/`ANON`/`SERVICE_ROLE` secrets exist by design.
